...
use SimpleUser\User as User;

/**
 * Class UserManager
 * @package SimpleUser
 *
 * Implements the UserProviderInterface{@inheritDoc}. Also proides utility functions used by
 * the UserController and the UserServiceProvider
 */
class UserManager implements UserProviderInterface
{
    ...
    /**
     * Test whether the current user has ROLE_USER
     *
     * @return bool
     */
    function isRoleUser() {
    ...
    /**
     * Test whether the current user has ROLE_ADMIN
     *
     * @return bool
     */
    function isRoleAdmin() {
        ...
        return $this->app['security.authorization_checker']->isGranted('ROLE_ADMIN');
    }

    function isAuthenticatedFully() {
        $token = $this->app['security.token_storage']->getToken();
        if (null === $token) {
            return false;
        }
        return $this->app['security.authorization_checker']->isGranted('IS_AUTHENTICATED_FULLY');
    }
    ...
    /**
     * This is a utility function which resets user passwords to the
     * values specified in the array below. This can be used when Simpleuser
     * or Silex has been upgraded and the password encoding algorithm
     * has changed (1.3 -> 2.0 introducing BCrypt). Only use in development
     * while unable to use normal password reset facilities. Allows reset of passwords
     * when the default encryption algorithm changes (Silex 1.3 -> 2.0). To use it add
     * this code into the start of loginAction of the UserController and fill in the
     * array with  entries 'user_id_value' => 'new_raw_user_password' for each user's
     * password you want to  change. When you select login, the passwords will be
     * changed. Remove the code after passwords have been reset and you can login
     * normally:-
     *
     *         if ($app['env'] == 'dev') {
     *             $passwords = array(
     *                 'user_id_value' => 'new_raw_user_password');
     *             $this->userManager->changeUserPasswords($passwords);
     *         };
     *
     * It is intended this will become an admin function to allow admin users to
     * reset user passwords if the email reset does not work properly as well.
     *
     * @param array $passwords array of passwords to change
     *
     */
    public function changeUserPasswords(array $passwords=array()) {
        if (!empty($passwords)) {
